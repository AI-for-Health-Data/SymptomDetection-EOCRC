...
MAX_INPUT_TOKENS = 8_192  # FIXED: was 12_000; paper states 8,192
...
MODERATE_SOURCE_PATTERNS: List[Tuple[str, str]] = [
    ...
    (r"\bphase ii\b", "phase II"),
    (r"\bphase 2\b", "phase 2"),
    (r"\bmulti[- ]center\b", "multi-center"),
]
...
VERY_LOW_SOURCE_PATTERNS: List[Tuple[str, str]] = [
    ...
    (r"\bin vivo\b", "in vivo (animal)"),
    (r"\bexpert consensus\b", "expert consensus"),
]

_TIER_CHECK_ORDER: List[Tuple[List[Tuple[str, str]], str]] = [
...
def classify_source_tier(text: str) -> Tuple[str, Optional[str]]:
    ...
    for patterns, tier in _TIER_CHECK_ORDER:
...
def find_phrase_spans(term: str, text: str) -> List[Tuple[int, int]]:
    """Return list of (start, end) spans for boundary-aware matches."""
    term_value = str(term).strip().lower()
    text_value = str(text).lower()
    if not term_value or len(term_value) < 3:
        return []
...
def prepare_source_filtered_evidence(
    ...
    for symptom in SYMPTOMS:
        ...
        for candidate_position, raw_passage in enumerate(
            ...
            tier, matched_marker = classify_source_tier(
                passage.get("text", "")
            )
...
For each symptom, in addition to label, confidence, and inference, also report:
...
- experiencer: who has the finding. Use one of:
  ...
  For symptoms, this should almost always be "patient".
...
and a GRADE-inspired source-design screening heuristic. SOURCE TIER labels
reflect keyword-based study-design classification and are NOT formal GRADE
certainty-of-evidence ratings. Formal GRADE requires outcome-level assessment
of risk of bias, inconsistency, indirectness, imprecision, and publication
bias across the complete body of evidence.
...
def complete_auxiliary_fields(parsed: Any) -> Any:
    """Normalize auxiliary and metadata fields; never create or change a
    symptom label."""
    if not isinstance(parsed, dict):
        return parsed
    result = dict(parsed)
    ...
    # --- FIX: Normalize metadata fields with defaults ---
    for symptom in SYMPTOMS:
...
def generate(
    ...
    """Generate text and return (text, output_truncated, input_truncated,
    full_prompt_tokens, used_prompt_tokens, output_tokens)."""
    import torch
...
def compute_label_probs(
    ...
    yes_token_ids: set = set()
    no_token_ids: set = set()
    for text_variant in ["Yes", "yes", "YES"]:
        yes_token_ids.update(
            tokenizer.encode(text_variant, add_special_tokens=False)
        )
    for text_variant in ["No", "no", "NO"]:
        no_token_ids.update(
            tokenizer.encode(text_variant, add_special_tokens=False)
        )
    ...
    for symptom in SYMPTOMS:
        found = False
        for pat in [f'"{symptom}"', f'"{symptom.lower()}"']:
            pat_idx = gen_lower.find(pat.lower())
            if pat_idx < 0:
                continue
            after = generated_text[pat_idx:]
            match = re.search(r'"(Yes|No|yes|no)"', after)
            if not match:
                continue
            label_char = pat_idx + match.start() + 1 
            prefix = generated_text[:label_char]
            prefix_toks = tokenizer.encode(prefix, add_special_tokens=False)
            pos = len(prefix_toks) - 1
 
            if 0 <= pos < len(probs):
                p_yes = sum(
                    probs[pos, tid].item()
                    for tid in yes_token_ids if tid < vocab_size
                )
                p_no = sum(
                    probs[pos, tid].item()
                    for tid in no_token_ids if tid < vocab_size
                )
                total = p_yes + p_no
                p_yes_norm = p_yes / total if total > 0 else 0.5
 
                if total > 0 and 0 < p_yes_norm < 1:
                    label_entropy = -(
                        p_yes_norm * math.log(p_yes_norm + 1e-10)
                        + (1 - p_yes_norm) * math.log(1 - p_yes_norm + 1e-10)
                    )
                else:
                    label_entropy = 0.0
                ...
                result[symptom] = {
                    ...
                    "label_entropy": round(label_entropy, 6),
                }
                found = True
            break
 
        if not found:
            # Could not locate the label token for this symptom
            result[symptom] = {
                "p_yes": -1.0,
                "p_no": -1.0,
                "p_yes_norm": -1.0,
...
def maybe_truncate(text: Any, max_chars: int = NOTE_CHAR_LIMIT) -> str:
    """Truncate note text from the RIGHT (end), keeping the beginning.

    Clinical notes typically place the most diagnostic-relevant content
    (Chief Complaint, HPI) at the beginning. Right-side truncation is
    consistent with the prompt design where the note appears before
    external context, so right-side input-token truncation removes
    retrieval context before removing note text.
    """
    value = "" if text is None else str(text)
    if len(value) <= max_chars:
        return value
    truncation_marker = "\n...[note truncated at character limit]...\n"
    available = max_chars - len(truncation_marker)
    return value[:available] + truncation_marker
...
def run_inference(
    ...
    print(
        "  GRADE precedence: HIGH > MODERATE > LOW > VERY_LOW "
        "(strongest first)"
    )
    print("  Patient note: before aliases/UMLS/retrieved context")
    print("  Note truncation: RIGHT-side (preserves beginning)")
    ...
    for dataframe_index, row in iterator:
        ...
        try:
            (
                ...
                full_tokens, used_tokens, output_tokens,
        ...
        if output_truncated or not isinstance(parsed, dict):
            ...
            try:
                (
                    ...
                    compact_prompt, tokenizer, model,
        ...
        if not isinstance(parsed, dict):
            ...
            try:
                repair_content, _, _, _, _, _ = generate(
        ...
        if isinstance(parsed, dict):
            ...
            if weak_symptoms:
                ...
                for start in range(0, len(weak_symptoms), 4):
                    ...
                    try:
                        retry_content, _, _, _, _, _ = generate(
                        ...
                        for symptom in batch:
                            ...
                            # Accept retry only if it improves grounding.
                            if (
        ...
        if isinstance(parsed, dict) and content:
            try:
                label_probs = compute_label_probs(
                    content, parsed, tokenizer, model
                )
                if label_probs:
                    label_prob_computed += 1
                else:
                    label_prob_failed += 1
            except Exception as error:
                label_prob_failed += 1
                print(f"  Label prob error for {note_identifier}: {error}")
        ...
        output_row["label_probs"] = json.dumps(label_probs, ensure_ascii=False)
    ...
    print("\n" + "-" * 78)
    print("SOURCE-TIER AUDIT (GRADE-hierarchical: HIGH checked first)")
    print("-" * 78)
...
def main() -> None:
    ...
    print("\n" + "=" * 78)
    print("EXPERIMENT 7 CONFIGURATION (SAGE Agent 1)")
    print("=" * 78)
    ...
    print("  Note truncation:  RIGHT-side (preserves beginning/HPI)")
    ...
    if args.phase in {"metrics", "all"}:
        if experiment_df is None:
            raise RuntimeError(
                "No Experiment 7 output is available for metrics."
            )
