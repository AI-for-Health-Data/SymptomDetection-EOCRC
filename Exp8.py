...
class CrossEncoderReranker:
    def __init__(self):
        ...
        self.model = CrossEncoder(CROSSENCODER_MODEL, device="cpu", max_length=512)
...
ONLY from the note text. Do NOT use retrieved passages as your
primary source — cite ONLY from the NOTE TEXT.
...
RETRIEVED BIOMEDICAL EVIDENCE (context only — cite from NOTE, not here):
{EVIDENCE_SECTION}
...
Output ONE flat JSON object. Every key maps to a string or number.
Do NOT use nested objects or sub-dictionaries as values.

CORRECT: {{"Abdominal pain": "Yes", "Abdominal pain confidence": 4,
           "Abdominal pain inference": "patient reports crampy abdominal pain x 3 weeks"}}
WRONG:   {{"Abdominal pain": {{"confidence": 4, "inference": "yes"}} }}
===================================
...
FOCUSED_RETRY_PROMPT = """You are an experienced gastroenterology clinician.
Re-examine ONLY the following symptoms from the patient's note:
{RETRY_SYMPTOMS}

Your PREVIOUS answers had LOW GROUNDING SCORES (evidence not in note text).

Previous answers:
{PREVIOUS_ANSWERS}

CRITICAL: Evidence MUST come from NOTE TEXT below. Do NOT cite passages.
If not found in note, answer "No" with confidence 2.

=== FLAT JSON ONLY ===
CORRECT: {{"Abdominal pain": "Yes", "Abdominal pain confidence": 4, "Abdominal pain inference": "..."}}
WRONG:   {{"Abdominal pain": {{"confidence": 4}} }}
=====================

GROUP B CONTEXT:
{GROUP_B_SECTION}

RE-RETRIEVED EVIDENCE (context only — cite from NOTE):
{EVIDENCE_SECTION}

SYNONYMS:
{ALIAS_SECTION_FOCUSED}

Patient NOTE TEXT:
<<NOTE_TEXT>>

Return flat JSON with ONLY:
{JSON_KEYS}""".strip()
...
def run_inference(notes_df, encoder, chunk_vecs, chunks, bm25,
    ...
    for idx, row in tqdm(notes_df.iterrows(), total=len(notes_df),
        ...
        if (idx + 1) % 50 == 0:
            pd.DataFrame(rows).to_csv("exp8v2_checkpoint.csv", index=False)
    ...
    exp_df.to_csv("exp8v2_outputs_raw.csv", index=False)
    ...
    pd.DataFrame(bleu_trajectory_log,
                 ...
                 ).to_csv("exp8v2_bleu_trajectory.csv", index=False)
...
def run_metrics(valid_df):
    ...
    metric_df.to_csv("exp8v2_note_level_metrics.csv", index=False)
    ...
    summary_df.to_csv("exp8v2_summary_table.csv", index=False)
...
if __name__ == "__main__":
    ...
    if args.phase in ("inference","all"):
        ...
        valid_df = run_inference(
            ...
            fh_chunks, fh_bm25, reranker,
            merged_vocab, group_b_lookup, tokenizer, model)
        valid_df.to_csv("exp8v2_valid_outputs.csv", index=False)

    if args.phase == "metrics":
        raw_df = pd.read_csv("exp8v2_outputs_raw.csv")
